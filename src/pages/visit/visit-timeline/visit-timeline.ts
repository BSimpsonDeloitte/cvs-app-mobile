...
export class VisitTimelinePage implements OnInit, OnDestroy {
  ...
  have5MinutesPassedSinceLastActivity(): boolean {
    if (!this.visit.tests.length) {
      return ((Date.parse(new Date().toISOString()) - Date.parse(this.visit.startTime)) / 1000 / 60) > 5;
    }
    if (this.visit.tests.length) {
      return ((Date.parse(new Date().toISOString()) - Date.parse(this.visit.tests[this.visit.tests.length - 1].endTime)) / 1000 / 60) > 5;
    }
  }
  ...
  waitTimeHandler(): void {
    if (this.canAddOtherWaitingTime(this.timeline)) {
      if (this.have5MinutesPassedSinceLastActivity()) {
        ...
        if (!this.visit.tests.length) {
          counterTime = this.activityService.counterTime - (Date.parse(new Date().toISOString()) - Date.parse(this.visit.startTime)) / 1000 / 60;
        }
        if (this.visit.tests.length) {
          counterTime = this.activityService.counterTime - (Date.parse(new Date().toISOString()) - Date.parse(this.visit.tests[this.visit.tests.length - 1].endTime)) / 1000 / 60;
        }
